...
stage/$(NAME)-scheduler: .godeps/.gpm_installed $(SOURCES) | stage
	$(GVP) in go build -o $@ -ldflags '-X\ main.version\ $(VER)' -v ./scheduler

stage/$(NAME)-shell-worker: .godeps/.gpm_installed $(SOURCES) | stage
	$(GVP) in go build -o $@ -ldflags '-X\ main.version\ $(VER)' -v ./shell-worker

## same, but shorter
build: test stage/$(NAME)-scheduler stage/$(NAME)-shell-worker
...
rpm: build
	...
	cp stage/$(NAME)-scheduler stage/$(NAME)-shell-worker stage/rpm/usr/bin/
	chmod 555 stage/rpm/usr/bin/*
